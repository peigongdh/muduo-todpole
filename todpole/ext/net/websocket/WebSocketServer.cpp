...
namespace muduo {
    namespace net {
        namespace ws {
            ...
            WsServer::WsServer(EventLoop *loop, const InetAddress &listenAddr,
                               const string &servername/* = "WsServer"*/)
                    : TcpServer(loop, listenAddr, servername) {
                setConnectionCallback(std::bind(&WsServer::onConnection, this, std::placeholders::_1));
                setMessageCallback(std::bind(&WsServer::onMessage, this, std::placeholders::_1, std::placeholders::_2,
                                             std::placeholders::_3));
            }
            ...
            void WsServer::handshake(const TcpConnectionPtr &conn, Buffer *buf, Timestamp receiveTime) {
                ...
                if (!context.parseRequest(buf, receiveTime) ||
                    context.request().method() != HttpRequest::kGet)// 解析失败 或者 不是Get请求
                {
            ...
            void WsServer::send(const TcpConnectionPtr &conn, const char *data, size_t size,
                                WsFrameType type/* = TEXT_FRAME*/) {
